 ...
 */
public class QuidemPostgisTest extends AbstractPostgisTest {

    @BeforeEach
    void setUp() throws SQLException {
        try (var connection = dataSource().getConnection()) {
            connection.createStatement().execute("CREATE TABLE countries (country VARCHAR, latitude DECIMAL, longitude DECIMAL, name VARCHAR);");
            for (Object[] row : ROWS) {
                var statement = connection.prepareStatement("INSERT INTO countries (country, latitude, longitude, name) VALUES (?, ?, ?, ?)");
                statement.setObject(1, row[0]);
                statement.setObject(2, row[1]);
                statement.setObject(3, row[2]);
                statement.setObject(4, row[3]);
                statement.execute();
            }
            connection.createStatement().execute("CREATE TABLE states (name VARCHAR, geom GEOMETRY);");
            for (Object[] row : STATE_ROWS) {
                var statement = connection.prepareStatement("INSERT INTO states (name, geom) VALUES (?, ST_GeomFromText(?))");
                statement.setObject(1, row[0]);
                statement.setObject(2, row[1]);
                statement.execute();
            }
            connection.createStatement().execute("CREATE TABLE parks (name VARCHAR, geom GEOMETRY);");
            for (Object[] row : PARK_ROWS) {
                var statement = connection.prepareStatement("INSERT INTO parks (name, geom) VALUES (?, ST_GeomFromText(?))");
                statement.setObject(1, row[0]);
                statement.setObject(2, row[1]);
                statement.execute();
            }
    ...
    @Test
    protected void runSpatialQueries() throws Exception {
        final String path = "sql/spatial.iq";
        final File inFile = new File("target/test-classes/" + path);
        final File outFile = new File("target/test-classes/" + path + ".out");
            ...
            final Quidem.Config config = Quidem.configBuilder()
                    ...
                    .withConnectionFactory(new ConnectionFactory() {
                        ...
                        public Connection connect(String name, boolean reference) throws Exception {
                            ...
                            // For some reason, double quotes are required.
                            info.setProperty("quoting", "DOUBLE_QUOTE");

                            // The column aliases returned by calcite are in upper case,
                            // while the ones returned by postgres through the jdbc adapter are in upper case.
                            // This is a workaround is not sufficient, and I had to modify the spatial.iq file.
                            // What is the correct way to deal with this?
                            info.setProperty("caseSensitive", "false");

                            // For some reason, setting the conformance to LENIENT in the PostgisSqlDialect is not sufficient.
                            // Is there a way to automatically set the conformance to LENIENT for all the tests?
                            info.setProperty("conformance", SqlConformanceEnum.LENIENT.name());
